...
// Profe tiene que tener antes el archivo de mascotas.txt si no lo descargo o no lo ha creado, no va  a correr adecuadamente el programa, y tiene que estar en la misma ubucacion gracias
...
void operaciones(int dor)
{
	...
	{
		printf("Dame el numero de mascotas adoptadas : "); scanf("%f",&monto);
		if (monto<=0) error(4);
